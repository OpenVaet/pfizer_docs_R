...
for (file in files) {
  ...
  if(!file.exists(paste0(dir, "/", csv_file))){
    ...
    # Write the data to a .CSV file in the "csv_data" sub-folder
    write.csv(data, paste0(dir, "/", csv_file), row.names = TRUE)
  }
}

# Get a list of all .xpt files in the "xpt_data" sub-folder
files <- list.files(path = "eua_data/xpt_data", pattern = "\\.xpt$")

# Create csv_data directory if it doesn't exist
dir <- "csv_data/eua_data"
if(!dir.exists(dir)){
  dir.create(dir)
}
