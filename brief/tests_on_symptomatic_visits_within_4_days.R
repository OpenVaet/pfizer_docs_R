...
# Loads the ADSL complete population.
adsl_file <- 'xpt_data/FDA-CBER-2021-5683-1066333-1067534_125742_S6_M5_c4591001-A-D-adsl.xpt'

# Reads the XPT file.
adsl_data <- read_xpt(adsl_file)
print(adsl_data, n=1)

# Filters out the rows where SUBJID is in the list of excluded subjects referenced by the ADRG.
...
filtered_adsl_data <- adsl_data[!(adsl_data$SUBJID %in% excluded_subjids), ]
print(filtered_adsl_data, n=1)

# Filters out the phase 1.
filtered_adsl_data <- filtered_adsl_data[filtered_adsl_data$PHASE != "Phase 1", ]
print(filtered_adsl_data, n=1)

# Filters out the rows where AGE < 16
filtered_adsl_data <- filtered_adsl_data[as.numeric(filtered_adsl_data$AGETR01) >= 16, ]
print(filtered_adsl_data, n=1)

# Filters out the rows where RANDNO == ""
filtered_adsl_data <- filtered_adsl_data[filtered_adsl_data$RANDNO != "", ]
print(filtered_adsl_data, n=1)

# Selects the desired columns
randomized_pop <- filtered_adsl_data[c(
