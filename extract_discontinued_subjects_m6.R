...
output_file <- "csv_data/discontinued_subjects_m6.csv"
if (file.exists(output_file)) {
  file.remove(output_file)
}
write(paste('M6_SUBJID', sep = ","), 
...
for(i in 1:length(trs)){
  ...
  if(length(tds) > 0){
    ...
    if(file_name == '125742_S1_M5_5351_c4591001 interim mth6 discontinued patients.pdf'){
      ...
      for (line in lines) {
        # Removes trailing newline character and extra spaces
        line <- str_trim(line)
        
        # Starts printing when line exactly matches "16.2.1.1"
        if (grepl("16\\.2\\.1\\.1", line)) {
          to_print <- 1
        }
        
        # Stops printing when line exactly matches "16.2.1.2"
        if (grepl("16\\.2\\.1\\.2", line)) {
          to_print <- 0
        }
        
        # Writes the line to the output file if the to_print flag is set
        if (to_print == 1) {
          # Checks if the line contains exactly an 8-digit number
          if (grepl("\\b\\d{8}\\b", line)) {
            # Extracts the 8-digit number
            number <- str_extract(line, "\\b\\d{8}\\b")
            write(number, file = output_file, append = TRUE)
          }
        }
      }
    }
