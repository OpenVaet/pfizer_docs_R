...
# Process symptoms data - create daily symptom records
...
# Process N-binding antibody tests
...
# Get all positive PCR tests after dose 1
post_dose1_pcr <- pcr_tests %>%
  inner_join(subjects, by = "SUBJID") %>%
  filter(TESTRESULT == "POS", TESTDATE > VAX101DT) %>%
...
# Convert SUBJID to character in randomized_pop to match flattened_simple
...
# By country
country_summary <- baseline_neg_subjects %>%
  ...
  pivot_wider(names_from = ARM, 
              values_from = c(n_total, n_with_sympt_pcr, pct_with_sympt_pcr),
              names_sep = "_")

cat("\n\nBy country and treatment arm:\n")
print(country_summary, n = 50)

# Show first few rows of the flattened data
cat("\n=== SAMPLE OF FLATTENED DATA (baseline negative only) ===\n")
baseline_neg_subjects %>%
  select(SUBJID, ARM, COUNTRY, VAX101DT, VAX102DT, 
         PCR_BASELINE_RESULT, NBIND_BASELINE_RESULT,
         FIRST_SYMPT_PCR_DATE, DAYS_TO_SYMPT_PCR) %>%
  arrange(FIRST_SYMPT_PCR_DATE) %>%
  print(n = 20)

# Distribution of time to symptomatic PCR
cat("\n=== TIME TO SYMPTOMATIC PCR (days from dose 1) ===\n")
baseline_neg_subjects %>%
  filter(!is.na(DAYS_TO_SYMPT_PCR)) %>%
  group_by(ARM) %>%
  summarise(
    n = n(),
    min = min(DAYS_TO_SYMPT_PCR),
    q25 = quantile(DAYS_TO_SYMPT_PCR, 0.25),
    median = median(DAYS_TO_SYMPT_PCR),
    q75 = quantile(DAYS_TO_SYMPT_PCR, 0.75),
    max = max(DAYS_TO_SYMPT_PCR),
    mean = mean(DAYS_TO_SYMPT_PCR),
    sd = sd(DAYS_TO_SYMPT_PCR),
    .groups = "drop"
  ) %>%
  print()
...
tte_data <- baseline_neg_subjects %>%
  mutate(
    ...
    days_to_event = case_when(
      !is.na(FIRST_SYMPT_PCR_DATE) ~ as.integer(FIRST_SYMPT_PCR_DATE - RANDDT),
      !is.na(UNBLNDDT) ~ as.integer(UNBLNDDT - RANDDT),
      TRUE ~ NA_integer_
    ),
    # Event indicator (1 = symptomatic PCR positive, 0 = censored)
    event = ifelse(!is.na(FIRST_SYMPT_PCR_DATE), 1, 0)
  ) %>%
  filter(!is.na(days_to_event), days_to_event >= 0) %>%
  select(SUBJID, ARM, COUNTRY, days_to_event, event, RANDDT, FIRST_SYMPT_PCR_DATE)
...
p1 <- ggplot(placebo_ci, aes(x = days_to_event, y = cumulative_incidence)) +
  ...
  labs(
    ...
    subtitle = paste0("N = ", nrow(placebo_data), " baseline-negative subjects"),
    x = "Days from Study Enrollment",
    y = "Cumulative Incidence (%)",
    caption = paste0("Total events: ", sum(placebo_data$event))
  ) +
  theme_minimal() +
  theme(
...
p2 <- ggplot(vaccine_ci, aes(x = days_to_event, y = cumulative_incidence)) +
  ...
  labs(
    ...
    subtitle = paste0("N = ", nrow(vaccine_data), " baseline-negative subjects"),
    x = "Days from Study Enrollment",
    y = "Cumulative Incidence (%)",
    caption = paste0("Total events: ", sum(vaccine_data$event))
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 14, face = "bold"),
