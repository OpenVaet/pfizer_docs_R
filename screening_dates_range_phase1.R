...
# Filters out subjects who aren't phase 1.
adsl_data <- adsl_data %>% filter(
  PHASEN == 1
)
print(paste('Total phase 1 subjects : ', nrow(adsl_data)))

# Selects the relevant columns for later use.
...
# Writes the selected data to a CSV file
...
# Generates a sequence of dates from the earliest date to the last protocol modification date
date_sequence <- seq(from = as.Date(first_protocol_modification_date), to = as.Date(last_protocol_modification_date), by = "day")

# Creates a data frame with the date sequence
daily_recruitment <- data.frame(RFICDT = date_sequence)

# Counts the number of subjects screened on each date within adsl_data
daily_counts <- adsl_data %>%
  group_by(RFICDT) %>%
  summarise(subjects_screened = n(), .groups = 'drop')

# Merges the sequence of dates with the count of subjects
daily_recruitment <- merge(daily_recruitment, daily_counts, by = "RFICDT", all.x = TRUE)

# Replaces NA values with 0 for days with no subjects screened
...
# Outputs to console for confirmation
...
# Counts the number of subjects randomized and categorize non-randomized subjects based on 'ARM' column
...
# Merges the randomization counts with the existing daily recruitment data frame
daily_recruitment <- merge(daily_recruitment, daily_randomization_counts, by = "RFICDT", all.x = TRUE)

# Replaces NA values with 0 for days with no subjects in each category
...
# Removes the 'Screened' column as it is no longer needed
daily_recruitment <- select(daily_recruitment, -subjects_screened)

# Writes the data to a CSV file with the new structure
...
# Converts the 'RFICDT' column to Date format for comparison
daily_recruitment$RFICDT <- as.Date(daily_recruitment$RFICDT)

daily_recruitment <- select(daily_recruitment, -Screen_Failure)

# Creates a new column to categorize the data based on the date criterion
daily_recruitment$Period <- ifelse(daily_recruitment$RFICDT < as.Date("2020-05-19"), "Before", "After")

# Excludes 'Screen_Failure' subjects and create a contingency table for the chi-square test
...
# Outputs the result of the chi-square test
print(chi_square_result)

# Extracts the relevant components from the chi-square test result
